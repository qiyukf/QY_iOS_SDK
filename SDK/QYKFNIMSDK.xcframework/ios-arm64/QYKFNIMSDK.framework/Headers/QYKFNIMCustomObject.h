...
/*
 ...
 由于 SDK 并不能预测上层 APP 的应用场景，所以 QYKFNIMCustomObject 采取消息透传的方式以提供给上层开发者最大的自由度
 即 SDK 只负责发送和收取由 QYKFNIMCustomObject 中 id<QYKFNIMCustomAttachment> attachment 序列化和反序列化后的字节流
 ...
 *  自定义消息对象附件协议
 */
@protocol QYKFNIMCustomAttachment <NSObject>
@required
 ...
 *
 *  @return 附件url
 *  @discussion 如果当前字段是云信的 http url 地址，那么需要调用 id<QYKFNIMResourceManager> 中 normalizeURLString: 方法将这个地址进行转 https 和 cdn 加速格式
 */
 ...
 *  自定义消息对象附件序列化协议
 */
@protocol QYKFNIMCustomAttachmentCoding<NSObject>
@required
 ...
 *  @return 自定义消息附件
 */
- (nullable id<QYKFNIMCustomAttachment>)decodeAttachment:(nullable NSString *)content;
@end
 ...
 *  用户自定义消息对象
 */
@interface QYKFNIMCustomObject : NSObject<QYKFNIMMessageObject>

/**
 *  用户自定义附件
 *  @discussion SDK负责将attachment通过encodeAttachment接口序列化后的结果进行透传
 */
@property(nullable, nonatomic, strong) id<QYKFNIMCustomAttachment>  attachment;
 ...
 */
+ (void)registerCustomDecoder:(id<QYKFNIMCustomAttachmentCoding>)decoder;
