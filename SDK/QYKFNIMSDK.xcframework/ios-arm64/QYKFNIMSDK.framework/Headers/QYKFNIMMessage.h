...
@class QYKFNIMMessageChatroomExtension;
@class QYKFNIMMessageAIConfigParams;
@class QYKFNIMMessageAIConfig;

/**
 *  消息送达状态枚举
 */
typedef NS_ENUM(NSInteger, QYKFNIMMessageDeliveryState){
    /**
     *  消息发送失败
     */
    QYKFNIMMessageDeliveryStateFailed,
    /**
     *  消息发送中
     */
    QYKFNIMMessageDeliveryStateDelivering,
    /**
     *  消息发送成功
     */
    QYKFNIMMessageDeliveryStateDeliveried
};

/**
 *  消息附件下载状态
 */
typedef NS_ENUM(NSInteger, QYKFNIMMessageAttachmentDownloadState){
    /**
     *  附件需要进行下载 (有附件但并没有下载过)
     */
    QYKFNIMMessageAttachmentDownloadStateNeedDownload,
    /**
     *  附件收取失败 (尝试下载过一次并失败)
     */
    QYKFNIMMessageAttachmentDownloadStateFailed,
    /**
     *  附件下载中
     */
    QYKFNIMMessageAttachmentDownloadStateDownloading,
    /**
     *  附件下载成功/无附件
     */
    QYKFNIMMessageAttachmentDownloadStateDownloaded
};

/**
 *  消息处理状态
 */
typedef NS_ENUM(NSInteger, QYKFNIMMessageStatus) {
    QYKFNIMMessageStatusNone        =   0,      //消息初始状态
    QYKFNIMMessageStatusRead        =   1,      //已读
    QYKFNIMMessageStatusDeleted     =   2       //已删除 (必须是 message status 最大值)
};

/**
 *  消息结构
 */
@interface QYKFNIMMessage : NSObject

/**
 *  消息类型
 */
@property (nonatomic,assign,readonly)       QYKFNIMMessageType messageType;
 ...
 *  消息所属会话
 */
@property (nullable,nonatomic,copy,readonly)       QYKFNIMSession *session;
...
/**
 *  消息文本
 *  @discussion 消息中除     QYKFNIMMessageTypeText 和 QYKFNIMMessageTypeTip 外，其他消息 text 字段都为 nil
 */
 ...
 *  消息附件内容
 */
@property (nullable,nonatomic,strong)                id<QYKFNIMMessageObject> messageObject;
 ...
 */
@property (nullable,nonatomic,strong)                QYKFNIMMessageSetting *setting;
 ...
 */
@property (nullable,nonatomic,strong)                QYKFNIMMessageApnsMemberOption *apnsMemberOption;
...
/**
 *  消息拓展字段
 *  @discussion 服务器下发的消息拓展字段，并不在本地做持久化，目前只有聊天室中的消息才有该字段(QYKFNIMMessageChatroomExtension)
 */
 ...
 *  消息投递状态 仅针对发送的消息
 */
@property (nonatomic,assign,readonly)       QYKFNIMMessageDeliveryState deliveryState;


/**
 *  消息附件下载状态 仅针对收到的消息
 */
@property (nonatomic,assign,readonly)       QYKFNIMMessageAttachmentDownloadState attachmentDownloadState;
 ...
 */
@property (nullable,nonatomic,strong,readonly)   QYKFNIMTeamMessageReceipt *teamReceiptInfo;
 ...
 *  发送者客户端类型
 */
@property (nonatomic,assign,readonly)   QYKFNIMLoginClientType senderClientType;
 ...
 *  消息处理状态
 */
@property (nonatomic, assign) QYKFNIMMessageStatus status;

/**
 *  消息机器人信息
 *  @discussion 设置和获取消息的机器人相关信息
 */
@property (nullable, nonatomic, strong) QYKFNIMMessageRobotInfo *robotInfo;

/**
 * 请求大模型的相关参数
 */
@property(nullable,nonatomic,strong,readwrite) QYKFNIMMessageAIConfigParams *sendAIConfig;

/**
 * 数字人消息配置
 */
@property(nullable,nonatomic,strong,readonly) QYKFNIMMessageAIConfig *aiConfig;
@end
