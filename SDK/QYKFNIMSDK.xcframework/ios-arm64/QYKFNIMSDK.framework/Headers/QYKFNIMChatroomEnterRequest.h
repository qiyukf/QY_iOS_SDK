 ...
 *  获取聊天室地址回调
 */
typedef void(^QYKFNIMRequestChatroomAddressesCallback)(NSError * __nullable error,NSArray<NSString *> * __nullable addresses);

/**
 *  获取聊天室地址方法
 */
typedef void(^QYKFNIMRequestChatroomAddressesHandler)(NSString *roomId,QYKFNIMRequestChatroomAddressesCallback callback);
 ...
 *  聊天室独立模式
 */
@interface QYKFNIMChatroomIndependentMode : NSObject
 ...
 *  参考代码：
 *
 *   [QYKFNIMChatroomIndependentMode registerRequestChatroomAddressesHandler:^(NSString * _Nonnull roomId, QYKFNIMRequestChatroomAddressesCallback  _Nonnull callback) {
 ...
 *
 */
+ (void)registerRequestChatroomAddressesHandler:(QYKFNIMRequestChatroomAddressesHandler)handler;

@end

typedef NS_ENUM(NSInteger, QYKFNIMChatroomLoginAuthType) {
        QYKFNIMChatroomLoginAuthTypeDefault = 0,
        QYKFNIMChatroomLoginAuthTypeDynamicToken = 1,
        QYKFNIMChatroomLoginAuthTypeThirdPart = 2,
};

/**
 *  进入聊天室请求
 */
@interface QYKFNIMChatroomEnterRequest : NSObject
 ...
 */
@property (nullable,nonatomic,strong)   QYKFNIMChatroomIndependentMode *mode;

/**
 * 鉴权方式
 */
@property (nonatomic, assign) QYKFNIMChatroomLoginAuthType loginAuthType;
 ...
 * 动态token回调
 */
@property (nonatomic, strong) QYKFNIMProvideChatroomDynamicTokenHandler dynamicTokenHandler;
