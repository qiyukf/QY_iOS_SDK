...
*/
typedef BOOL(^QYKFNIMHistoryMessageFilterBlock)(QYKFNIMMessage *message);
 ...
 */
@interface QYKFNIMMessageSearchOption : NSObject
...
/**
 *  查询的消息类型
 *  @discusssion 消息类型组合，默认只搜索文本类型, 只有在 allMessageTypes 为 NO 时有效，取值范围为: QYKFNIMMessageType 枚举类型
 */
 ...
 *  检索服务器历史消息选项 (服务器)
 */
@interface QYKFNIMHistoryMessageSearchOption : NSObject
 ...
 */
@property (nullable,nonatomic,strong)      QYKFNIMMessage      *currentMessage;
...
*/
@property (nullable,nonatomic,strong)  QYKFNIMHistoryMessageFilterBlock customFilter;
 ...
 *  @discussion 搜索条件: 时间在(startTime,endTime) 内(不包含)，类型为 messageTypes （或全类型） ，且匹配 searchContent 或 fromIds 的一定数量 (limit) 消息
 */
@interface QYKFNIMMessageServerRetrieveOption : NSObject
 ...
 *  @discussion 搜索条件: 时间在(startTime,endTime) 内(不包含)，类型为 messageTypes （或全类型） ，且匹配 searchContent 或 fromIds 的一定数量 (limit) 消息
 */
@interface QYKFNIMMessageFullKeywordSearchOption : NSObject
 ...
 *  @discussion 搜索条件: 时间在(startTime,endTime) 内(不包含)，类型为 messageTypes （或全类型） ，且匹配 searchContent 或 fromIds 的一定数量 (limit) 消息
 */
@interface QYKFNIMMessageFullKeywordSearchOrderByTimeOption : NSObject
...
@interface QYKFNIMGetMessagesDynamicallyParam : NSObject <NSCopying>

/**
 *  会话session。
 */
@property (nonatomic, strong)      QYKFNIMSession  *session;
...
- (QYKFNIMGetMessagesDynamicallyParam *) createStandardizedParam;
