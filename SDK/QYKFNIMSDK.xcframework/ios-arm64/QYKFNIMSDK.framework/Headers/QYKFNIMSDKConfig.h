...
@class QYKFNIMNotificationObject;
@class QYKFNIMMessage;
@class QYKFNIMChatroomCdnTrackInfo;
@protocol QYKFNIMMsgIndexProduceDelegate;
@class QYKFNIMLogDesensitizationConfig;

/**
 *  SDK 配置委托
 */
@protocol QYKFNIMSDKConfigDelegate <NSObject>
 ...
 *  @return 是否忽略
 */
- (BOOL)shouldIgnoreNotification:(QYKFNIMNotificationObject *)notification;
 ...
 *  @return 是否忽略
 */
- (BOOL)shouldIgnoreMessage:(QYKFNIMMessage *)message;

/**
 * 聊天室cdn统计回调，回调时间间隔为QYKFNIMSDKConfig.cdnTrackInterval
 */
- (void)onChatroomCdnTrack:(QYKFNIMChatroomCdnTrackInfo *)trackInfo forRoom:(NSString *)roomID;
 ...
 *  NIM SDK 配置项目
 */
@interface QYKFNIMSDKConfig : NSObject
...
/**
 *  自动登录重试次数
 *  @discusssion 默认为 0。即默认情况下，自动登录将无限重试。设置成大于 0 的值后，在没有登录成功前，自动登录将重试最多 maxAutoLoginRetryTimes 次，如果失败，则抛出错误 (QYKFNIMLocalErrorCodeAutoLoginRetryLimit)。
 */
 ...
 *  配置项委托
 */
@property (nullable,nonatomic,weak)    id<QYKFNIMSDKConfigDelegate>    delegate;
 ...
 *  @discussion 设置该值后 SDK 产生的数据(包括聊天记录，但不包括临时文件)都将放置在这个目录下，如果不设置，所有数据将放置于 $Document/QYKFNIMSDK目录下
 *              该配置项必须在 QYKFNIMSDK 任一一个 sharedSDK 方法调用之前调用，否则配置无法生效
 */
 ...
 *  消息索引生成委托
 */
@property (nullable, nonatomic)    id<QYKFNIMMsgIndexProduceDelegate> msgIndexProducer;
 ...
 * 日志脱敏配置
 */
@property (nonatomic, copy) QYKFNIMLogDesensitizationConfig *logDesensitizationConfig;
