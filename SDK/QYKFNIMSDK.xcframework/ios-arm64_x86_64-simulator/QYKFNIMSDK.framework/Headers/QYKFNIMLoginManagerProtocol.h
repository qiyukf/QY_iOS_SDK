...
#ifndef QYKFNIMDeprecated
#define QYKFNIMDeprecated(msg) __attribute__((deprecated(msg)))
#endif
 ...
 */
typedef void(^QYKFNIMLoginHandler)(NSError * __nullable error);
 ...
 *  @param error 执行结果,如果成功error为nil
 */
typedef void(^QYKFNIMLoginGetServerTimeHandle)(NSError * __nullable error, QYKFNIMServerTime *time);

/**
 *  登录步骤枚举
 */
typedef NS_ENUM(NSInteger, QYKFNIMLoginStep)
{
    /**
     *  连接服务器
     */
    QYKFNIMLoginStepLinking = 1,
    /**
     *  连接服务器成功
     */
    QYKFNIMLoginStepLinkOK,
    /**
     *  连接服务器失败
     */
    QYKFNIMLoginStepLinkFailed,
    /**
     *  登录
     */
    QYKFNIMLoginStepLogining,
    /**
     *  登录成功
     */
    QYKFNIMLoginStepLoginOK,
    /**
     *  登录失败
     */
    QYKFNIMLoginStepLoginFailed,
    /**
     *  开始同步
     */
    QYKFNIMLoginStepSyncing,
    /**
     *  同步完成
     */
    QYKFNIMLoginStepSyncOK,
    /**
     *  连接断开
     */
    QYKFNIMLoginStepLoseConnection,
    /**
     *  网络切换
     *  @discussion 这个并不是登录步骤的一种,但是UI有可能需要通过这个状态进行UI展现
     */
    QYKFNIMLoginStepNetChanged,
    /**
     *  主动退出
     */
    QYKFNIMLoginStepLogout,
};

/**
 *  SDK 认证模式
 */
typedef NS_ENUM(NSInteger, QYKFNIMSDKAuthMode)
{
    /**
     *  未定义
     *  @discussion SDK 未调用任何登录接时或在 IM/聊天室 模式下调用 logout 接口后变化为未定义模式
     */
    QYKFNIMSDKAuthModeUndefined = 0,
    /**
     *  通过 IM 服务器鉴权
     *  @discussion 调用 QYKFNIMLoginManager login/autoLogin 接口进行登录即为 IM 鉴权模式
     */
    QYKFNIMSDKAuthModeIM,
    /**
     *  聊天室单独鉴权
     *  @discussion 调用 QYKFNIMChatroomManager 进入聊天室接口时设置 QYKFNIMChatroomIndependentMode 即为聊天室单独聊天鉴权模式
     */
    QYKFNIMSDKAuthModeChatroom,
};
 ...
 */
typedef NS_ENUM(NSInteger, QYKFNIMSDKAuthType)
{
    QYKFNIMSDKAuthTypeDefault = 0,
    QYKFNIMSDKAuthTypeDynamicToken = 1,
    QYKFNIMSDKAuthTypeThirdParty = 2,
};

/**
 *  被踢下线的原因
 */
typedef NS_ENUM(NSInteger, QYKFNIMKickReason)
{
    /**
     *  被另外一个客户端踢下线 (互斥客户端一端登录挤掉上一个登录中的客户端)
     */
    QYKFNIMKickReasonByClient = 1,
    /**
     *  被服务器踢下线
     */
    QYKFNIMKickReasonByServer = 2,
    /**
     *  被另外一个客户端手动选择踢下线
     */
    QYKFNIMKickReasonByClientManually   = 3,
};

/**
*  多端登陆的状态
*/
typedef NS_ENUM(NSInteger, QYKFNIMMultiLoginType){
    /**
     *  目前已经有其他端登陆
     */
    QYKFNIMMultiLoginTypeInit   =   1,
    /**
     *  其他端上线
     */
    QYKFNIMMultiLoginTypeLogin  =   2,
    /**
     *  其他端下线
     */
    QYKFNIMMultiLoginTypeLogout =   3,
};

/**
 *  IM 登录相关回调接口，包括登录状态、多端登录、被踢下线、数据同步状态等事件的监听回调。
 */
@protocol QYKFNIMLoginManagerDelegate <NSObject>

@optional
/**
 *  被其他端（服务端/客户端）踢下线的回调。<br>
 *
 *  @deprecated 请使用- (void)onKickout:(QYKFNIMLoginKickoutResult *)result;
 *
 *  @par 参数说明：
 *  <table>
 ...
 *  </table>
 */
- (void)onKick:(QYKFNIMKickReason)code clientType:(QYKFNIMLoginClientType)clientType QYKFNIMDeprecated("Use -onKickout: instead");
 ...
 *  @par 参数说明：
 *  result：被踢后收到的回调信息，包括被踢下线的原因（QYKFNIMKickReason）和将其踢下线的设备端的类型（QYKFNIMLoginClientType）
 *  @par 相关接口：
 ...
 */
- (void)onKickout:(QYKFNIMLoginKickoutResult *)result;
 ...
 *  @par 触发时机：
 *  从调用登录 IM 接口开始，到登录结束（包括成功和失败）之间符合任一状态（QYKFNIMLoginStep）时触发回调。
 *  @par 注意事项：
 ...
 */
- (void)onLogin:(QYKFNIMLoginStep)step;
 ...
 *  @deprecated 请使用- (void)onMultiLoginClientsChangedWithType:(QYKFNIMMultiLoginType)type;
 */
...
*/
- (void)onMultiLoginClientsChangedWithType:(QYKFNIMMultiLoginType)type;
 ...
 *  @deprecated 请使用 QYKFNIMSDKConfigDelegate dynamicTokenForAccount:
 */
 ...
 */
@protocol QYKFNIMLoginManager <NSObject>
...
- (void)login:(NSString *)account
   ...
   completion:(QYKFNIMLoginHandler)completion;
 ...
 *
 *  @param account    帐号
 *  @param token      令牌 (authType为QYKFNIMSDKAuthTypeDefault时有效)
 *  @param authType   登陆类型，取值范围为QYKFNIMSDKAuthType
 *  @param loginExt   登录自定义字段 (authType为QYKFNIMSDKAuthTypeThirdParty时有效；authType为QYKFNIMSDKAuthTypeThirdPartyDynamic时，loginExt通过NIMSDKConfigDelegate中的dynamicLoginExtForAccount:来获取)
...
/**
 *  自动登录 IM。
 *  @deprecated 请使用- (void)autoLogin:(QYKFNIMAutoLoginData *)loginData;
 *  @par 使用建议：
 ...
 */
- (void)autoLogin:(QYKFNIMAutoLoginData *)loginData;
 ...
 *  @par 参数说明：
 *  completion：登出完成的回调。回调中的 QYKFNIMLoginHandler error 只是指明和服务器的交互流程中可能出现的错误，但不影响后续的流程。 如用户登出时发生网络错误导致服务器没有收到登出请求，客户端仍可以登出(切换界面，清理数据等)，但会出现推送信息仍旧会发到当前手机的问题。
 *  @par 相关回调：
 *  [onLogin:](https://doc.yunxin.163.com/docs/interface/messaging/iOS/doxygen/Latest/zh/d5/dc6/protocol_n_i_m_login_manager_delegate-p.html#a70055366358595d63b0e1c07aedeb55f) 登录状态回调<br>
 *  [onMultiLoginClientsChangedWithType:](https://doc.yunxin.163.com/docs/interface/messaging/iOS/doxygen/Latest/zh/d5/dc6/protocol_n_i_m_login_manager_delegate-p.html#a83f001bb0a5c477a72178f1c0b2585df) 多端登录设备列表
 */
- (void)logout:(nullable QYKFNIMLoginHandler)completion;
...
- (void)kickOtherClient:(QYKFNIMLoginClient *)client
             completion:(nullable QYKFNIMLoginHandler)completion;
 ...
 */
- (QYKFNIMSDKAuthMode)currentAuthMode;

/**
 *  查询当前账号登录的客户端信息（QYKFNIMLoginClient），将返回当前登录的的设备列表，包括客户端类型、操作系统、登录时间等信息。
 *
 *  @par 返回： 
 ...
 * completion：查询完成的回调
 */
- (void)queryServerTimeCompletion:(QYKFNIMLoginGetServerTimeHandle)completion;
 ...
 *  delegate：登录相关的委托
 */
- (void)addDelegate:(id<QYKFNIMLoginManagerDelegate>)delegate;
 ...
 *  delegate：登录相关的委托
 */
- (void)removeDelegate:(id<QYKFNIMLoginManagerDelegate>)delegate;
@end
