...
@class QYKFNIMSDKOption;
@class QYKFNIMQChatOption;
@class QYKFNIMServerSetting;
 ...
 */
typedef void(^QYKFNIMArchiveLogsHandler)(NSError *error, NSString *path);

typedef void(^QYKFNIMUploadLogsHandler)(NSError *error, NSString *path);


/**
 *  SDK 核心类，提供初始化 SDK，获取各个服务能力接口，获取当前版本等接口
 */
@interface QYKFNIMSDK : NSObject
/**
 *  获取 NIM SDK 实例。
 *  @par 调用时机：
 ...
 *  如需要同时配置 PushKit 推送证书名称，请使用 - (void)registerWithOption:(QYKFNIMSDKOption *)option; 接口。
 *  @par 调用时机：
 *  建议在应用启动时初始化 NIM SDK。
 *  @par 参数说明：
 *  <table>
 ...
 */
- (void)registerWithOption:(QYKFNIMSDKOption *)option;
 ...
 *  option：圈组选项，推送证书名称
 */
- (void)qchatWithOption:(QYKFNIMQChatOption *)option;
 ...
 */
- (void)archiveLogs:(QYKFNIMArchiveLogsHandler)completion;
 ...
 */
- (void)uploadLogs:(QYKFNIMUploadLogsHandler _Nullable)completion;
...
- (void)uploadLogsWithAttach:(nullable NSString *)attach
                  ...
                  completion:(QYKFNIMUploadLogsHandler _Nullable)completion;
 ...
 */
@property (nonatomic,strong)           QYKFNIMServerSetting    *serverSetting;
 ...
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMLoginManager>   loginManager;

/**
 *  聊天管理类，提供消息发送、消息查询、消息撤回等相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMChatManager>     chatManager;

/**
 *  会话管理类，负责消息、最近会话的读写和管理，提供获取/删除会话消息、设置会话消息已读等相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMConversationManager>  conversationManager;

/**
 *  媒体管理类，提供语音录制和播放相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMMediaManager>    mediaManager;

/**
 *  群组管理类,负责群组的操作:创建,拉人,踢人,同步等
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMTeamManager>    teamManager;

/**
 *  超大群管理类，提供超大群成员管理、超大群消息发送、超大群资料管理等相关接口
 */
@property (nonatomic,strong,readonly) id<QYKFNIMSuperTeamManager> superTeamManager;

/**
 *  好友管理类，提供用户信息管理和好友关系管理相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMUserManager>      userManager;

/**
 *  系统通知管理类，提供发送、标记、删除系统通知等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMSystemNotificationManager>    systemNotificationManager;

/**
 *  APNS 推送管理类，提供APNs 离线推送服务相关配置接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMApnsManager> apnsManager;

/**
 *  资源管理器，提供下载、搜索、查询资源等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMResourceManager> resourceManager;

/**
 *  聊天室管理类，提供进出聊天室、发送聊天室消息、聊天室成员管理、聊天室队列服务、聊天室标签等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMChatroomManager> chatroomManager;

/**
 *  文档转码管理类，提供查询文档转码信息、删除转码文档等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMDocTranscodingManager> docTranscodingManager;

/**
 *  事件订阅管理类，提供发布、查询、订阅事件等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMEventSubscribeManager> subscribeManager;

/**
 *  智能机器人管理类，提供获取机器人信息等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMRobotManager> robotManager;

/**
 *  红包管理类，提供获取红包 token 的接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMRedPacketManager> redPacketManager;

/**
 *  广播消息管理类，提供广播消息监听等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMBroadcastManager> broadcastManager;

/**
 *  反垃圾管理类，提供本地反垃圾检查接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMAntispamManager> antispamManager;

/**
 *  通用音视频信令管理类，提供信令相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMSignalManager> signalManager;

/**
 *   透传代理管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMPassThroughManager> passThroughManager;


/**
 *   Thread Talk（扩展消息）管理类，提供收藏消息、快捷评论等接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMChatExtendManager> chatExtendManager;

/**
 *   索引管理类，提供消息检索相关接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMIndexManager> indexManager;

/**
 *  圈组管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatManager> qchatManager;

/**
 *  圈组APNS推送管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatApnsManager> qchatApnsManager;

/**
 *  圈组服务器管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatServerManager> qchatServerManager;

/**
 *  圈组频道管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatChannelManager> qchatChannelManager;

/**
 *  圈组身份组管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatRoleManager> qchatRoleManager;

/**
 *  圈组消息管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatMessageManager> qchatMessageManager;

/**
 *  圈组消息扩展管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatMessageExtendManager> qchatMessageExtendManager;

/**
 *  圈组rtc管理类
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMQChatRTCChannelManager> qchatRTCChannelManager;

/**
 *  定制化接口调用管理类，提供调用定制化 API 接口
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMCustomizedAPIManager> customizedAPIManager;

/**
 *   系统状态
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMSystemStateManager> systemStateManager;

/**
 * 数字人
 */
@property (nonatomic,strong,readonly)   id<QYKFNIMAIManager> aiManager;
