 ...
 *  消息配置
 */
@interface QYKFNIMMessageSetting : NSObject
/**
 *  消息是否允许在消息历史中拉取（圈组支持）
 *  @discussion 默认为YES。 正常而言所有消息都会出现在通过 QYKFNIMConversationManager 调用(fetchMessageHistory:option:result:)返回的结果中，但是可以通过设置这个值来使得消息不出现在这其中。
 */
