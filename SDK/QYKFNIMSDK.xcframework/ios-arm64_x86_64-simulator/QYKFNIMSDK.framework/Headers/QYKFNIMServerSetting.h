...
typedef NS_ENUM (NSInteger, QYKFNIMHandshakeType){
    QYKFNIMHandshakeTypeBasics = 0,
    QYKFNIMHandshakeTypeAdvanced,
};

typedef NS_ENUM (NSInteger, QYKFNIMLinkAddressType){
    QYKFNIMLinkAddressTypeIpv4 = 0,  //仅使用IPv4
    QYKFNIMLinkAddressTypeIpv6,      //仅使用IPv6
    QYKFNIMLinkAddressTypeAuto,      //自动选择
    QYKFNIMLinkAddressTypeIpv6First  //IPv6优先
};

typedef NS_ENUM (NSInteger, QYKFNIMServerEnv){
    QYKFNIMServerEnvProd = 0,       // 线上环境
    QYKFNIMServerEnvDev,            // 测试环境
};

/**
 *  云信服务器配置
 */
@interface QYKFNIMServerSetting : NSObject

/**
* 握手协议类型。基础的类型仅支持设置module和version.使用RSA和RC4加密方式。
* 高级类型，可以通过配置encryptConfig来指定使用的加密类型。默认：QYKFNIMHandshakeTypeBasics
*/
@property (nonatomic,assign)            QYKFNIMHandshakeType handshakeType;

/**
 服务器环境(默认线上环境)
 */
@property (nonatomic,assign)            QYKFNIMServerEnv    env;
...
* 握手加密配置
*/
@property (nonatomic,readonly)          QYKFNIMEncryptionConfig   *encryptConfig;
...
*/
@property (nonatomic, assign) QYKFNIMLinkAddressType lbsLinkAddressType;
