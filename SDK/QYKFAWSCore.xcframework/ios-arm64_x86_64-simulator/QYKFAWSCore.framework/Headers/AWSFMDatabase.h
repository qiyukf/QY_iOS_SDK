...
#import "AWSFMDatabasePool.h"
    ...
    #define QYKFAWSFMDBAutorelease(__v) ([__v autorelease]);
    #define QYKFAWSFMDBReturnAutoreleased QYKFAWSFMDBAutorelease

    #define QYKFAWSFMDBRetain(__v) ([__v retain]);
    #define QYKFAWSFMDBReturnRetained QYKFAWSFMDBRetain

    #define QYKFAWSFMDBRelease(__v) ([__v release]);

    #define QYKFAWSFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define QYKFAWSFMDBAutorelease(__v)
    #define QYKFAWSFMDBReturnAutoreleased(__v) (__v)

    #define QYKFAWSFMDBRetain(__v)
    #define QYKFAWSFMDBReturnRetained(__v) (__v)

    #define QYKFAWSFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define QYKFAWSFMDBDispatchQueueRelease(__v)
    #else
        #define QYKFAWSFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^QYKFAWSFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in QYKFAWSFMDB are:
 ...
 - [QYKFAWSFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [QYKFAWSFMDB mailing list](http://groups.google.com/group/fmdb)
...
@interface QYKFAWSFMDatabase : NSObject  {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(QYKFAWSFMDBExecuteStatementsCallbackBlock)block;
...
- (QYKFAWSFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (QYKFAWSFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (QYKFAWSFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (QYKFAWSFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (QYKFAWSFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)QYKFAWSFMDBUserVersion;

+ (SInt32)QYKFAWSFMDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(FMDatabase *adb) {
        ...
        QYKFAWSFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one QYKFAWSFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to QYKFAWSFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using QYKFAWSFMDB you will not need to interact directly with `FMStatement`, but rather with `<FMDatabase>` and `<FMResultSet>` only.
...
@interface QYKFAWSFMStatement : NSObject {
    NSString *_query;
    long _useCount;
    BOOL _inUse;
}
