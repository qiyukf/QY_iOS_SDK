...
FOUNDATION_EXPORT NSString *const QYKFAWSCognitoCredentialsProviderErrorDomain;
typedef NS_ENUM(NSInteger, QYKFAWSCognitoCredentialsProviderErrorType) {
    QYKFAWSCognitoCredentialsProviderErrorUnknown,
    QYKFAWSCognitoCredentialsProviderIdentityIdIsNil,
    QYKFAWSCognitoCredentialsProviderInvalidConfiguration,
    QYKFAWSCognitoCredentialsProviderInvalidCognitoIdentityToken,
    QYKFAWSCognitoCredentialsProviderCredentialsRefreshTimeout,
};

@class QYKFTask<__covariant ResultType>;
@class QYKFCancellationTokenSource;

/**
 An AWS credentials container class.
 */
@interface QYKFAWSCredentials : NSObject <NSCopying>
 ...
 */
@protocol QYKFAWSCredentialsProvider <NSObject>
 ...
 */
- (QYKFTask<QYKFAWSCredentials *> *)credentials;
...
/**
 ...
 We strongly discourage embedding AWS credentials in your production apps because they can be easily extracted and abused. Consider using `QYKFAWSCognitoCredentialsProvider`.
 */
@interface QYKFAWSStaticCredentialsProvider : NSObject <QYKFAWSCredentialsProvider>
 ...
 */
@interface QYKFAWSBasicSessionCredentialsProvider: NSObject <QYKFAWSCredentialsProvider>
...
@interface QYKFAWSAnonymousCredentialsProvider : NSObject <QYKFAWSCredentialsProvider>
...
/**
 An QYKFAWSCredentialsProvider that uses Amazon Cognito to fetch temporary credentials tied to an identity.
 ...
 3. Developer authenticated identities: Uses your own QYKFAWSCognitoCredentialsProviderHelper to establish identity +
 Cognito (and optionally STS) to establish credentials.
 */
@interface QYKFAWSCognitoCredentialsProvider : NSObject <QYKFAWSCredentialsProvider>
 ...
 */
@property (nonatomic, strong, readonly) id<QYKFAWSCognitoCredentialsProviderHelper> identityProvider;
...
/**
 Initializer for credentials provider with pre-created `QYKFAWSCognitoCredentialsProviderHelper`. Use this method when using developer authenticated identities.

 @param regionType The region in which your identity pool exists.
 @param identityProvider Implementation of QYKFAWSCognitoCredentialsProviderHelper which is responsible for acquiring identity id and (optionally) OpenId Connect token.
 */
- (instancetype)initWithRegionType:(AWSRegionType)regionType
                  identityProvider:(id<QYKFAWSCognitoCredentialsProviderHelper>)identityProvider;

/**
 Initializer for credentials provider with pre-created `QYKFAWSCognitoCredentialsProviderHelper`. Only use this method if you need to set your IAM roles client side and use developer authenticated identities
...
- (void)setIdentityProviderManagerOnce:(id<QYKFAWSIdentityProviderManager>)identityProviderManager;
